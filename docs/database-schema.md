...
| `tests`, `test_categories` | `tests`: full CRUD via `/admin/catalog`, including `primary_image_url`, a `media`-backed gallery, and `normal_range_template` (see [normal-range.ts](../lib/types/normal-range.ts)). `test_categories`: full CRUD via `/admin/categories`, including `default_image_url` (fallback photo for tests with none of their own — `lib/data/tests.ts`). Both public read (falls back to mock data — see [decisions-log.md](./decisions-log.md)) |
...
| `packages`, `package_tests` | Full CRUD via `/admin/packages`, including `primary_image_url` upload, a `media`-backed gallery, and a checklist UI for `package_tests` (delete-all-then-insert-selected on save). Public read + detail page + bookable |
| `media` | Used for three cases now via one generalized action pair (`lib/actions/media.ts`'s `upsertMedia`/`deleteMedia`, `components/admin/MediaGalleryForm.tsx`): `entity_type='landing'` (homepage hero carousel, `/admin/site-content`), `entity_type='test'`/`'package'` (multi-photo galleries on each item's edit page, rendered via `components/ui/PhotoGallery.tsx`). Separate from `tests.primary_image_url`/`packages.primary_image_url` (single "main" image, uploaded via `lib/actions/upload-image.ts`) — see [decisions-log.md](./decisions-log.md) for why both exist |
