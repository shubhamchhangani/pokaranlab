...
| 4 | Catalog management | **Mostly built.** `tests` (`app/admin/catalog/{new,[id]}`) and `packages` (`app/admin/packages/{new,[id]}`, includes a `package_tests` linking checklist) both have full create/edit/delete + raw-JSON `custom_fields` editors + a single primary-image upload (`lib/actions/upload-image.ts`, to `public-media`). `test_categories` (`app/admin/categories`) has add/delete but no rename. **Still missing:** multi-photo galleries / reordering (the full `media` table is otherwise untouched by any UI). |
